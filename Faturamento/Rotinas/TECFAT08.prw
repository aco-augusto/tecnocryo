...
Function u_FAT8Central()
	...
	oBrwZZ2 :addLegend( "ZZ2_STATUS == '3' ", "GRAY"	 ,'Cobrança Finalizada'	)
	...
	oBrwSC6 :SetFilterDefault( "LEFT(SC6->C6_PRODUTO,4) == '0023' .AND. xFilial('SC6')==SC6->C6_FILIAL" )
	oBrwSC6:Refresh(.T.)
	...
	oSD2Relation:AddRelation( oBrwZZ2  , oBrwSC6 , { {"C6_FILIAL","ZZ2_FILIAL"		}, {"C6_YCODLOC","ZZ2_CODLOC"}} )				  	
...
Static Function F08LoadZZ2(cCliDe, cLojDe, cCliAte, cLojAte)
	...
	BeginSql Alias qryZZ2
		...
		ZZ2_STATUS 	   IN ('1', ' ')
		%Exp:cWhere% 
		Order by A1_NOME, ZZ2_LOJA, ZZ2_PRDAPL
...
Static Function F08GrvPed( qryZZ2 )
	...
	WHILE (qryZZ2)->( ! EOF() ) 
		...
		IF ZZ2->ZZ2_DTAAPL > dIniPeriodo  .OR. ZZ2->( IIF( ! Empty(ZZ2_ULTRET), ZZ2_ULTRET > dIniPeriodo .and. ZZ2_ULTRET < dFimPeriodo, .F.) ) //trata locações com período parcial, pelo iníco tardio ou pelo termino antecipado
			...
			IF Len( aDev ) > 0
				...
				FOR nI := 1 TO Len( aDev )
					...
					cMsg 		:= "( dias de locação " + StrZero( aDev[nI][2] - dIniPeriodo, 2 ) + " )" //Calcula o número de dias de locação, considerando a data de devolução com a data de início do período

					//Procura se o produto já foi incluído para a quantidade de dias em questão.
					nPos 		:= aScan( aPreItens, {|x,y| aDev[nI][ nPrd ] == x[nPrd] .and. x[nDias] == aDev[nI][nDias] .and. ( qryZZ2 ) -> ( ZZ2_SERIE + ZZ2_NOTREM + ZZ2_CLIENT + ZZ2_LOJA ) == x[nMsg] + x[nCli] + x[nLoj] })
					aDev[nI][4] := cMsg + " NF Rem: " + aDev[nI][4]
					aDev[nI][5] := aDev[nI][2] - dIniPeriodo
			...
			ELSE
				aDev := {}
				aAdd( aDev, { (qryZZ2)->ZZ2_PRDAPL, stod( (qryZZ2)->ZZ2_DTAAPL ), (qryZZ2)->ZZ2_QTDAPL, ZZ2 -> ( ZZ2_SERIE + ZZ2_NOTREM ),  0, ZZ2->ZZ2_CLIENT, ZZ2->ZZ2_LOJA, ZZ2->ZZ2_CODLOC, dFimPeriodo } )
				...
				//Procura se o produto já foi incluído para a quantidade de dias em questão.
				nPos 		:= aScan( aPreItens, {|x,y| (qryZZ2)->ZZ2_PRDAPL == x[nPrd] .and. x[nDias] == nQtdDias .and. ( qryZZ2 ) -> ( ZZ2_SERIE + ZZ2_NOTREM + ZZ2_CLIENT + ZZ2_LOJA ) == x[nMsg] + x[nCli] + x[nLoj]  })
				aDev[nI][4] := cMsg + (qryZZ2)->ZZ2_NOTREM
				aDev[nI][5] := dFimPeriodo - aDev[nI][2]
		...
		IF ZZ2->ZZ2_DTAAPL <= dIniPeriodo //Trata locação período integral
			...
			//Procura se o produto já foi incluído para a quantidade de dias em questão.
			nPos 	 := aScan( aPreItens, {|x,y| (qryZZ2)->ZZ2_PRDAPL == x[nPrd] .and. nQtdDias == x[nDias]  .and. ( qryZZ2 ) -> ( ZZ2_SERIE + ZZ2_NOTREM + ZZ2_CLIENT + ZZ2_LOJA ) == x[nMsg] + x[nCli] + x[nLoj] })

			//Monta array pre-itens para geração do pedido de venda
			IF nPos > 0
				aPreItens[nPos][nQtd] += (qryZZ2)->ZZ2_SALDO //nQuantFull
			ELSE 

				aAdd( aPreItens, {} )
				aAdd( aPreItens[ Len(aPreItens) ], ( qryZZ2 ) -> ZZ2_PRDAPL ) 	//01
				aAdd( aPreItens[ Len(aPreItens) ], ( qryZZ2 ) -> ZZ2_DTAAPL ) 	//02
				aAdd( aPreItens[ Len(aPreItens) ], ( qryZZ2 ) -> ZZ2_SALDO ) 	//03
				aAdd( aPreItens[ Len(aPreItens) ], ( qryZZ2 ) -> ( ZZ2_SERIE + ZZ2_NOTREM ) )	//04
				aAdd( aPreItens[ Len(aPreItens) ], nQtdDias )					//05
				aAdd( aPreItens[ Len(aPreItens) ], ( qryZZ2 ) -> ZZ2_CLIENT )					//06
				aAdd( aPreItens[ Len(aPreItens) ], ( qryZZ2 ) -> ZZ2_LOJA )					//07
				aAdd( aPreItens[ Len(aPreItens) ], ( qryZZ2 ) -> ZZ2_CODLOC )					//08
				aAdd( aPreItens[ Len(aPreItens) ], dFimPeriodo )				//09
...
Static Function F08InsPedido( aPreItens, dFimPeriodo ) 
	...
	For nI := 1 TO Len( aPreItens )
		nSc6Recno := SeekSC6Recno( aPreItens[1][nCodL], DtoS(aPreItens[1][nDtGer]) ) 
		IF nSc6Recno > 0
			EXIT
		ENDIF
	Next
	...
	IF nSc6Recno > 0 .and. lExcPed
		...
		aCabExc := {}
		cSC6Key	:= xFilial("SC6") + SC6->C6_NUM

		Posicione( "SC6", 1, cSC6Key, "Found()" )

		aAdd(aCabExc, {"C5_NUM", SC6->C6_NUM, Nil} )

		WHILE SC6 -> ( ! Eof() .AND. xFilial("SC6") + SC6->C6_NUM == cSC6Key)
	...
	ELSEIF nSc6Recno > 0 .AND. ! Empty( SC5->C5_NOTA )		
		...
		Aviso( "Atenção", "Existe um pedido "+ SC5->C5_NUM + " faturado para a locação " + aPreItens[1][nCodL] + " na data de geração "+ DtoC(aPreItens[1][nDtGer]) + " e não é possível gerar mais de um pedido para o mesmo período de locação.", { "Ok" })
		Return
	...
	IF nSc6Recno == 0 
		...
		FOR nI := 1 TO Len( aPreItens )
			...
			IF Empty(cPrdLoc)
				Aviso("Atenção!", "Não encontrado AMARRAÇÃO (locação: " + aPreItens[nI][nCodL]  + ") para o produto  " + rTrim(aPreItens[nI][nPrd] ) + ' aplicado ao cliente ' + SA1->A1_COD + "-" + SA1->A1_LOJA+ "  e NÃO SERÁ COBRADA A LOCAÇÃO. Verifique.", { "Ok" } )
				LOOP
			ENDIF
			...
			IF nPrecoVenda == 0
				...
				Aviso( "Atenção", "O produto " + Rtrim(cPrdLoc) + " não foi encontrado na tabela de preço " + cTabSa1 + ", para o cliente " + SA1->( A1_COD + '-' +  A1_LOJA) + ". Não será gerada cobrança de locação para o mesmo. Verifique.", { "Ok" })
			...
			aadd( aTmp, { "C6_YGERLOC"	,	aPreItens[nI][nDtGer]				, NIL } )  
...
Static Function F08GetDevol( dIniPeriodo, dFimPeriodo )
	...
	BeginSql Alias cNextArea
		...
		SELECT D1_COD, 
		(CASE WHEN D1_YDTRET = '' THEN D1_EMISSAO ELSE D1_YDTRET END) D1_EMISSAO, 
		D1_QUANT, D1_FORNECE, D1_LOJA //D1_DOC, D1_SERIE, D1_QUANT, D1_EMISSAO 
		FROM %table:SD1% SD1
		WHERE 
		SD1.%NotDel% AND 
	...
	dbEval( { || aAdd( aDevolucao, { D1_COD, D1_EMISSAO, D1_QUANT, ZZ2 -> ( ZZ2_SERIE + ZZ2_NOTREM ),  0, D1_FORNECE, D1_LOJA, ZZ2->ZZ2_CODLOC, dFimPeriodo } ) } )
	//									1  , 	2	   , 	3	 ,  4								,  5, 	  6		,   7    , 8			  , 9
	...
	IF ( qryZZ2 ) -> ( ! EOF() )
		...
		BEGIN TRANSACTION
			...
			WHILE ( qryZZ2 ) -> ( ! EOF() )
				...
				//				cChave 	:= (qryZZ2)->ZZ2_SERIE+(qryZZ2)->ZZ2_NOTREM+(qryZZ2)->ZZ2_CLIENT+(qryZZ2)->ZZ2_LOJA+(qryZZ2)->ZZ2_PRDAPL
				//				Posicione("ZZ2", 4, xFilial("ZZ2") + cChave, "Found()")
				//				lNewRecno := ZZ2->( ! Found() )
				//zz2Recno := ZZ2->( Recno() )//SeekZZ2Recno( (qryZZ2)->ZZ2_SERIE, (qryZZ2)->ZZ2_NOTREM, (qryZZ2)->ZZ2_CLIENT, (qryZZ2)->ZZ2_LOJA, (qryZZ2)->ZZ2_PRDAPL )

				//				IF ZZ2 -> ( found() ) //zz2Recno > 0
				//					lNewRecno := .F.
				//					//					ZZ2-> ( dbGoTo( zz2Recno ) )
				//				ELSE
				//					lNewRecno := .T.
